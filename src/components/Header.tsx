...
import { UtensilsCrossed, LayoutDashboard, RotateCcw, ShieldCheck, Activity } from 'lucide-react';

interface HeaderProps {
  activeView: 'guest' | 'dashboard';
  onViewChange: (view: 'guest' | 'dashboard') => void;
  onResetDemo: () => void;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20 gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center text-neutral-950 shadow-lg shadow-emerald-950/50">
              <UtensilsCrossed className="w-5 h-5 font-black" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-lg font-black tracking-tight text-white">
                  its<span className="text-emerald-400">my</span>booking
                </span>
                <span className="hidden sm:inline-block px-2 py-0.5 text-[10px] font-bold bg-neutral-800 text-neutral-300 rounded border border-white/10 uppercase">
                  itsmyapp.co.uk
                </span>
              </div>
              <p className="text-[11px] text-neutral-400 hidden sm:block">
                UK Hospitality Booking & Kitchen Pacing Engine
              </p>
            </div>
          </div>

          {/* View Switcher Tabs (Guest vs Dashboard) */}
          <div className="flex items-center bg-neutral-900 p-1 rounded-xl border border-white/10">
            <button
              type="button"
              onClick={() => onViewChange('guest')}
              className={`min-h-[38px] px-3 sm:px-4 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                activeView === 'guest'
            ...
            >
              ...
              <span className="hidden xs:inline">Guest</span> Booking
            </button>
            ...
            >
              ...
              <span>Operator</span> <span className="hidden sm:inline">FOH</span>
            </button>
          </div>
